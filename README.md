...
3. Add it as a child node in your scenes to start making API calls.
...
# Initialize the Cocobase SDK
var cocobase = Cocobase.new()

func _ready() -> void:
    add_child(cocobase)
    # Initialize with your API key
    cocobase.init("YOUR_API_KEY_HERE")
...
```gdscript
# Create a document
var doc_data = {"name": "John", "score": 100}
...
```gdscript
# Login user
var login_data = {"email":"user@example.com", "password":"mypassword"}
cocobase.loginUser(login_data)

# Create user
var user_data = {"email":"newuser@example.com", "password":"mypassword"}
cocobase.createUser(user_data)

# Get current user info
cocobase.getCurrentUser()

# Update current user info
cocobase.updateCurrentUser()

# Send verification email
cocobase.sendVerificationEmail()

# Verify email
cocobase.verifyEmail("verification_token_here")

# Resend verification email
cocobase.resendVerificationEmail()
```
Notes
-----
...
*   Make sure your API key is kept private and **never hardcode it in public projects**.
    
*   The SDK currently supports JSON payloads only.
    
*   This plugin is compatible with **Godot 3.4+**.
...
3.  Call the desired API functions from your script, such as creating collections or fetching documents.
